...
function takomiTranscript(outputTokens: number): string {
  return (
    [
      { type: "session", id: "takomi-account-session" },
      { type: "model_change", modelId: "gpt-5.6-terra" },
      {
        type: "message",
        id: "takomi-account-message",
...
describe("UsageService", () => {
  it.live("reads configured and disabled accounts once across shared and aliased homes", () =>
    Effect.gen(function* () {
      ...
      const piAlias = NodePath.join(home, "pi-alias");
      yield* Effect.promise(async () => {
        ...
        await NodeFSP.mkdir(NodePath.join(piHome, "agent", "sessions"), { recursive: true });
        await NodeFSP.symlink(piHome, piAlias, "junction");
        await NodeFSP.writeFile(
          NodePath.join(piHome, "agent", "sessions", "session.jsonl"),
          takomiTranscript(17),
        );
      });
            ...
            settings: {
              ...
              providerInstances: {
                ...
                [ProviderInstanceId.make("pi")]: {
                  ...
                  environment: [{ name: "PI_CODING_AGENT_DIR", value: piHome, sensitive: false }],
                },
                [ProviderInstanceId.make("pi-alias")]: {
                  driver: ProviderDriverKind.make("pi"),
                  config: { homePath: piAlias },
                },
              },
            },
          }),
      ...
      assert.strictEqual(totalOutputTokens(summary), 53);
      ...
      assert.strictEqual(
        sources.filter((source) => source.fingerprint.provider === "takomi").length,
        1,
      );
    }).pipe(Effect.scoped),
  );
