...
import { isModelSelectionUnavailable, normalizeProviderDispatchModes } from "../lib/modelOptions";
import {
...
export function useThreadOutboxDrain(): void {
  ...
  const sendQueuedMessage = useCallback(
    async (queuedMessage: QueuedThreadMessage, thread: EnvironmentThreadShell) => {
      ...
      const queuedSettings = resolveQueuedThreadSettings(
        queuedMessage,
        thread,
        serverConfig.providers,
      );
      if (isModelSelectionUnavailable(serverConfig, queuedSettings.modelSelection)) {
      ...
      const currentSettings = resolveQueuedThreadSettings(
        queuedMessage,
        settings,
        currentConfig.providers,
      );
      const sendModes = normalizeProviderDispatchModes({
        config: currentConfig,
        modelSelection: currentSettings.modelSelection,
        runtimeMode: currentSettings.runtimeMode,
      ...
      const sendSettings = { ...currentSettings, ...sendModes };
