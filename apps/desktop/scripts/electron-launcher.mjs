// This file mostly exists because we want dev mode to say "Takomi Code (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Takomi Code (Dev)" : "Takomi Code (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.jstarfilms.takomicode.dev.${devBundleIdSuffix || "local"}`
  : "com.jstarfilms.takomicode";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["takomi-code-dev"] : ["takomi-code"];
const LAUNCHER_VERSION = 19;
const developmentMacIconPngPath = NodePath.join(
  repoRoot,
  "assets",
  "takomi",
  "takomi-icon-1024.png",
);
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Takomi Code name.
