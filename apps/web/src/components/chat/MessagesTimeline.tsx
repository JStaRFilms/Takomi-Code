...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  let paintedExpandedTakomiToolCallIds = expandedTakomiToolCallIds;
  if (listIdentityRef.current !== listIdentityKey) {
    ...
    paintedExpandedTakomiToolCallIds = new Set();
    ...
    setExpandedTakomiToolCallIds(paintedExpandedTakomiToolCallIds);
  }
  ...
  const sharedState = useMemo<TimelineRowSharedState>(
    () => ({
      ...
      onOpenAgents,
      expandedTakomiToolCallIds: paintedExpandedTakomiToolCallIds,
    ...
    [
      ...
      onOpenAgents,
      paintedExpandedTakomiToolCallIds,
      onToggleTakomiToolCall,
      onSelectTakomiToolCall,
    ],
  );
...
function WorkEntryRow({
  ...
  const { expandedTakomiToolCallIds, onToggleTakomiToolCall, onSelectTakomiToolCall } =
    use(TimelineRowCtx);
  if (isTakomiPresentation(workEntry)) {
    const toolCallId = workEntry.toolCallId ?? workEntry.id;
    return (
      <TakomiToolCallCard
        entry={workEntry}
        expanded={expandedTakomiToolCallIds.has(toolCallId)}
        onToggle={() => onToggleTakomiToolCall(toolCallId)}
