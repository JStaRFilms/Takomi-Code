...
function deriveTurnFolds(input: {
  ...
  for (const [turnId, group] of groupsByTurnId) {
    ...
    for (const entry of group.entries) {
      ...
      if (
        (entry.kind === "message" && keepAssistantMessagesVisible) ||
        (entry.kind === "work" && entry.entry.presentation !== undefined)
      ) {
        continue;
      }
...
export function deriveMessagesTimelineRows(input: {
  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    if (timelineEntry.kind === "work") {
      if (
        timelineEntry.entry.presentation?.namespace === "takomi" ||
        timelineEntry.entry.presentation?.namespace === "takomi-flow" ||
