...
## Bounded recovery and backpressure

Transient connection failures retry indefinitely, but the delay is capped at
16 seconds and resets after a connection remains stable for 30 seconds. A
foreground wake can restore at most eight subscriptions concurrently per
environment, preventing a resumed client from starting every stream at once.

Server subscription tails are bounded by item count and serialized bytes. When
a durable shell, thread, or server-configuration tail exceeds its 256-item or
2 MiB budget, the server replaces the incomplete tail with an authoritative
snapshot instead of retaining an unbounded replay. Other subscription adapters
close only the slow client when the same queue budget is exhausted. Every RPC
response also passes through a 4 MiB WebSocket output budget; crossing it closes
the client with code 1013 so native output cannot grow without bound.
